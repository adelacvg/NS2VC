...
# def normalize_cvec(cvec):
#     return 2 * ((cvec - CVEC_MIN) / (CVEC_MAX - CVEC_MIN)) - 1
# def denormalize_cvec(norm_cvec):
#     return ((norm_cvec+1)/2)*(CVEC_MAX-CVEC_MIN)+CVEC_MIN


def normalize_to_neg_one_to_one(img):
    return normalize_tacotron_mel(img)
    return img * 2 - 1

def unnormalize_to_zero_to_one(t):
    return denormalize_tacotron_mel(t)
    return (t + 1) * 0.5
...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def sample(self, spec, classes, cond_scale = 6., rescaled_phi = 0.7):
        batch_size, image_size, channels = classes.shape[0], self.image_size, self.channels
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        return sample_fn(classes, spec.shape, cond_scale, rescaled_phi)
    ...
    def p_losses(self, x_start, t, classes=None, noise = None):
        b, c, T = x_start.shape
        noise = default(noise, lambda: torch.randn_like(x_start))
    ...
    def forward(self, img, *args, **kwargs):
        b, c, T, device = *img.shape, img.device
