...
import torchaudio
from modules.xtransformers import ContinuousTransformerWrapper, RelativePositionBias
import glob
...
class CrossQKVAttentionLegacy(nn.Module):
    """
    A module which performs QKV attention. Matches legacy QKVAttention + input/output heads shaping
    """
    ...
    def forward(self, q, kv, mask=None, rel_pos=None):
        """
        Apply QKV attention.
        ...
        return a.reshape(bs, -1, q_length)
...
class CrossAttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        # split heads before split qkv
        self.attention = CrossQKVAttentionLegacy(self.num_heads)
    ...
    def forward(self, x, cond, mask=None):
        ...
        kv = self.kv(self.norm1(cond))
        h = self.attention(q, kv, mask, self.relative_pos_embeddings)
        h = self.proj_out(h)
        return (x + h).reshape(b, c, *spatial)
