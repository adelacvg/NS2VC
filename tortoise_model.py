...
from modules.ttts_utils import CrossAttentionBlock, normalization, AttentionBlock
from modules.perceiver import PerceiverResampler

from ns2vc_model import Transformer_blocks
...
class ResBlock(TimestepBlock):
    ...
    def forward(self, x, emb, cond_latent=None):
        h = self.in_layers(x)
...
class DiffusionLayer(TimestepBlock):
    def __init__(self, model_channels, dropout, num_heads):
        ...
        # self.cross_attn = CrossAttentionBlock(model_channels, num_heads, relative_pos_embeddings=True)

    def forward(self, x, time_emb, cond_latent):
        y = self.resblk(x, time_emb)
        y = self.attn(y)
        # y = self.cross_attn(y, cond_latent)
        return y
...
class DiffusionTts(nn.Module):
    def __init__(
        ...
        # transformer network.
        self.code_embedding = nn.Linear(1,1)
        # self.code_embedding = nn.Embedding(in_tokens, model_channels)
        self.code_converter = nn.Linear(1,1)
        ...
        self.code_norm = normalization(model_channels)
        # self.latent_conditioner = nn.Linear(1,1)
        ...
        # self.contextual_embedder = nn.Linear(1,1)
        ...
        self.cond_proj = nn.Linear(model_channels, 2*model_channels) 
        # self.cond_latent_proj = nn.Linear(in_channels, model_channels) 
        self.perceiver = PerceiverResampler(dim=model_channels, depth=3, dim_context=None, num_latents=1, dim_head=64, heads=8, ff_mult=4, use_flash_attn=False)
        # self.transformer_blocks = Transformer_blocks(in_latent_channels,hidden_dim=model_channels)
    ...
    def forward(self, x, timesteps, aligned_conditioning=None, conditioning_latent=None, precomputed_aligned_embeddings=None, conditioning_free=False, return_code_pred=False):
        ...
        # unused_params.extend(list(self.contextual_embedder.parameters()))
        # unused_params.extend(list(self.latent_conditioner.parameters()))

        time_emb = self.time_embed(timestep_embedding(timesteps, self.model_channels))

        # cond_latent = self.cond_perceiver(conditioning_latent.transpose(1,2)).transpose(1,2)
        cond_latent = self.contextual_embedder(conditioning_latent)

        cond_emb = self.perceiver(cond_latent.transpose(1,2)).squeeze(1)
        cond_emb = self.cond_proj(cond_emb)
        cond_scale, cond_shift = torch.chunk(cond_emb, 2, dim=1)
        code_emb = self.code_norm(code_emb) * (1 + cond_scale.unsqueeze(-1)) + cond_shift.unsqueeze(-1)

        code_emb = self.cross_attn(code_emb, cond_latent) + code_emb
        # cond_latent = self.cond_latent_proj(cond_latent.transpose(1,2)).transpose(1,2)
        # time_emb = time_emb + cond_emb
        code_emb = self.conditioning_timestep_integrator(code_emb, time_emb, cond_latent)
        x = self.inp_block(x)
...
if __name__ == '__main__':
    ...
    aligned_latent = torch.randn(2,512,123)
